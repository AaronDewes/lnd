...
func NewBitcoindBackend(t *testing.T, minerAddr string, txindex,
	...
	ppid := os.Getppid()
	rpcPort := rpctest.NextAvailablePortForProcess(ppid)
	zmqBlockPort := rpctest.NextAvailablePortForProcess(ppid)
	zmqTxPort := rpctest.NextAvailablePortForProcess(ppid)
...
func init() {
	// Go starts a new sub process for each test package, but we want to
	// have unique listening addresses across all packages in case things
	// run parallel. That's why we use the parent process id to generate
	// unique addresses.
	parentProcess := os.Getppid()
	rpctest.ListenAddressGenerator = func() (string, string) {
		return rpctest.GenerateProcessUniqueListenerAddresses(
			parentProcess,
		)
	}
}
